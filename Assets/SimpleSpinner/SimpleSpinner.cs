...
namespace Assets.SimpleSpinner
{
    ...
    public class WorldLoadingUIManager : MonoBehaviour
    {
        [Header("Rotation")]
        ...
        public AnimationCurve RainbowAnimationCurve =
            AnimationCurve.Linear(0, 0, 1, 1);
        ...
        public void Update()
        {
            ...
            if (Rotation)
            {
                transform.localEulerAngles = new Vector3(
                    0, 0,
                    -360f * RotationAnimationCurve.Evaluate(
                        (RotationSpeed * Time.time + _period) % 1
                    )
                );
            }
            ...
            if (Rainbow)
            {
                _image.color = Color.HSVToRGB(
                    RainbowAnimationCurve.Evaluate(
                        (RainbowSpeed * Time.time + _period) % 1
                    ),
                    RainbowSaturation,
                    1
                );
